...
//   float binsPass[] = {1,2,3,4,5,6,7};//
   ...
   TH1F *Ratio=new TH1F("ratio OS/SS", "ratio", binnum, bins);
   TH1F *SSTimesRatio= new TH1F("SSTimesRatio", "SSTimesRatio", binnum, bins);
   ...
   std::vector<TH1F*> mtFailPassH;
   std::vector<TH1F*> mtFailPassSSH;
   ...
   for (int k=0; k<nbhist; ++k){
        ...
        ostringstream HmtFailPassH; HmtFailPassH << "mtFailPassH" << k;
        ostringstream HmtFailPassSSH; HmtFailPassSSH << "mtFailPassSSH" <<k;
        ...
        mtFailPassH.push_back(new TH1F (HmtFailPassH.str().c_str(),"InvMa", binnum, bins)); mtFailPassH[k]->Sumw2();
        mtFailPassSSH.push_back(new TH1F (HmtFailPassSSH.str().c_str(), "InvMa", binnum, bins)); mtFailPassSSH[k]->Sumw2();
...
//cout<<correction<<" "<<sf_iso<<" "<<sf_trg<<" "<<sf_id<<" "<<sf_trk<<" "<<LumiWeights_12->weight(npu)<<endl;
	...
	TLorentzVector mymet;
	   ...
	   //}
           if (fillOS && cut_zeta && mt<40 && tauIsolation) {//Pass OS
               ...
               mtFailPassH[k]->Fill(var, aweight);
           ...
           if (fillSS && cut_zeta && mt<40 &&  tauIsolation) {//Pass SS
               ...
               mtFailPassSSH[k]->Fill(var, aweight);
           }
    ...
    TDirectory *OSDir=fout->mkdir("OS");
    TDirectory *SSDir=fout->mkdir("SS");
    ...
    std::cout<<"nbhist="<<nbhist<<std::endl;
    for (int k=0; k<nbhist; ++k){
       if (tes==1)
       ...
       mtFailH[k]->Write();
       OSDir->cd();
       mtFailPassH[k]->SetBinContent(binnum+1,0);
       mtFailPassH[k]->SetBinError(binnum+1,0);
       mtFailPassH[k]->SetBinContent(0,0);
       mtFailPassH[k]->SetBinError(0,0);
       for (int l=0; l<binnum;++l){
           if (mtFailPassH[k]->GetBinContent(l+1)<0)
              mtFailPassH[k]->SetBinContent(l+1,0);
       }
       mtFailPassH[k]->SetName(name.c_str()+postfix);
       mtFailPassH[k]->Write();
       passSS->cd();
       ...
       SSDir->cd();
       mtFailPassSSH[k]->SetBinContent(binnum+1,0);
       mtFailPassSSH[k]->SetBinError(binnum+1,0);
       mtFailPassSSH[k]->SetBinContent(0,0);
       mtFailPassSSH[k]->SetBinError(0,0);
       for (int l=0; l<binnum;++l){
           if (mtFailPassSSH[k]->GetBinContent(l+1)<0)
              mtFailPassSSH[k]->SetBinContent(l+1,0);
       }
       mtFailPassSSH[k]->SetName(name.c_str()+postfix);
       mtFailPassSSH[k]->Write();
    }
    fout->Close();
} 
